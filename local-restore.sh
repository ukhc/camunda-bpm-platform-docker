...
kubectl exec -i $POD -- /usr/bin/mysql -u root -padmin camunda < ./backup/$BACKUP_FOLDER/database/camunda-bpm-platform-dump.sql
# validate
# kubectl exec -it $POD -- /usr/bin/mysql -u root -padmin -e 'use baget;show tables;'

##########################

echo "restart the camunda-bpm-platform deployment..."
kubectl scale --replicas=0 deployment camunda-bpm-platform
echo "wait a moment..."
sleep 5
kubectl scale --replicas=1 deployment camunda-bpm-platform
...
do
	...
	if [ "$isPodReady" != "true" ]; then
		...
		if [ "$isPodReadyCount" -gt "100" ]; then
			...
			echo "waiting...camunda-bpm-platform pod is not ready...($isPodReadyCount)"
			sleep 2
		fi
	fi
done
